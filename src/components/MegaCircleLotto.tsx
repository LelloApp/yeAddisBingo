...
interface MegaCircleLottoProps {
  ...
  totalPot: number;
  drawTime: Date; // e.g. 18:00 or 19:00 EAT
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-3xl p-4 shadow-2xl relative overflow-hidden">
        <div className="flex justify-between items-start">
          <div>
            <div className="inline-flex items-center gap-1.5 px-3 py-0.5 rounded-full bg-amber-500/10 border border-amber-500/20 text-amber-400 text-[11px] font-bold">
              ...
              <span>{isSuperBonus ? 'DAILY SUPER BONUS LOTTO' : 'ADDIS DAILY LOTTO'}</span>
            </div>
            <h2 className="text-xl font-black text-white mt-1">{title}</h2>
            <p className="text-xs text-slate-400">
              {isSuperBonus
                ? 'Drawn daily at 7:00 PM • Funded by 20% of Owner 24h Cuts'
                : 'Drawn daily at 6:00 PM • 100 ETB / Token Entry'}
            </p>
          </div>
        ...
        <div className="grid grid-cols-2 gap-2 mt-3.5 pt-3 border-t border-slate-800/80">
          <div className="bg-slate-950/80 border border-amber-500/30 rounded-2xl p-2.5">
            <div className="text-[10px] uppercase font-bold text-amber-400">Current Winning Pot</div>
            <div className="text-xl font-black text-white font-mono mt-0.5">
              {totalPot.toLocaleString()} ETB
            </div>
            <div className="text-[10px] text-slate-400">
              {totalPot < 1000 ? '⚠️ Min 1,000 ETB to draw' : '✅ Ready to draw'}
            </div>
          </div>
          ...
          <div className="bg-slate-950/80 border border-blue-500/30 rounded-2xl p-2.5">
            ...
            <div className="text-[10px] text-slate-400">
              {activeTokens.length} Tokens in Wheel
            </div>
          </div>
        </div>
                  ...
                  <div>
                    {winner ? (
                      <div>
                        ...
                        <div className="text-[10px] text-slate-400">
                          Ticket #{winner.tokenNumber} • Agent: {winner.adminName}
                        </div>
                      </div>
                    ) : (
                      <div className="italic text-slate-500 text-xs">
                        {isCurrentlyDrawing ? '⚡ Decelerating & Seeking Winner...' : 'Pending draw...'}
                      </div>
                    )}
                  </div>
                </div>
